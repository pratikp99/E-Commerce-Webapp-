...
public class addServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			catDao = new categoryDAO(DatabaseUtil.getConnection());
		...
		try {
			pdao = new productDAO(DatabaseUtil.getConnection());
		} catch (ClassNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		if (operation.trim().equals("addCategory")) {
			...
			categoryModel category = new categoryModel(categoryName, part.getSubmittedFileName());
			...
			if (flag) {
				message = new messageModel("Category added successfully!!", "success", "alert-success");
			} else {
			...
			productModel product = new productModel(pName, pDesc, pPrice, pDiscount, pQuantity, part.getSubmittedFileName(),
					categoryType);
			...
			if (flag) {
				message = new messageModel("Product added successfully!!", "success", "alert-success");
			} else {
				message = new messageModel("Something went wrong! Try again!!", "error", "alert-danger");
			}
			session.setAttribute("message", message);
			...
			if (part.getSubmittedFileName().isEmpty()) {
				...
				categoryModel category = new categoryModel(cid, name, image);
				catDao.updateCategory(category);
			} else {
				categoryModel category = new categoryModel(cid, name, part.getSubmittedFileName());
				catDao.updateCategory(category);
				String path = request.getServletContext().getRealPath("/") + "Product_imgs" + File.separator
						+ part.getSubmittedFileName();
				try {
			...
			message = new messageModel("Category updated successfully!!", "success", "alert-success");
			session.setAttribute("message", message);
			...
			if (part.getSubmittedFileName().isEmpty()) {
				String image = request.getParameter("image");
				productModel product = new productModel(pid, name, description, price, discount, quantity, image, cid);
				pdao.updateProduct(product);
			} else {

				productModel product = new productModel(pid, name, description, price, discount, quantity,
			...
			message = new messageModel("Product updated successfully!!", "success", "alert-success");
			session.setAttribute("message", message);
